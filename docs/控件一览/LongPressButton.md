...
> **重要**：当前实现中，[`LongPressButton`](../../Product/UI/UI.Common/SensingControl/LongPressButton.cs) 只有在 XML 中配置了 `<PathSource>` 且能加载到图片时，才会创建显示内容并绑定触摸事件。如果省略 `<PathSource>`，控件内部为空，**不会响应任何触摸/点击操作**。
...
3. **必须配置 `<PathSource>` 序列帧路径**（当前实现强依赖，否则控件不会创建内容和绑定事件）；
4. 准备好序列帧图片资源（如需要长按动画效果）。
...
`ClickEvent` 在用户长按并松开时触发。如果配置了序列帧动画，只有松开时动画已经播放到最后一帧，才会触发事件。
...
- 用户按下按钮，`PathSource` 中的序列帧动画开始逐帧播放；
- 用户松开按钮时判断当前帧是否已经播放到最后一帧：
  - 如果已经到达最后一帧，触发 `ClickEvent`；
  - 如果未到达最后一帧，动画反向回退到第一帧，**不触发** `ClickEvent`；
- 触发事件的不是“按下达到固定时长”，而是“松开时序列帧已播完”。
...
- **当前实现必须配置 `<PathSource>`**，否则控件不会创建内容和绑定事件，表现为既看不到也按不了；
- 如果只需要透明热区，也需要配置一个只含单张图片的 `<PathSource>` 路径；
- 检查 `Opacity` 是否设置过低；
- 检查 `ZIndex` 是否被其他控件遮挡。
